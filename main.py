...
# ----------------------------------------------------------------------
# Helper: display a member as a proper mention
# ----------------------------------------------------------------------
def format_member_display(guild: discord.Guild, user_id: int, username_hint: str = None) -> str:
    """Return a proper mention string for a user."""
    member = guild.get_member(user_id)
    if member:
        return member.mention
    # fallback to ID mention if not in guild
    return f"<@{user_id}>"
...
async def daily_backup():
    ...
    embed = discord.Embed(title="📦 نسخة احتياطية يومية", color=discord.Color.purple(), timestamp=datetime.utcnow())
    embed.description = "تم إنشاء نسخة احتياطية تلقائية لبيانات السجلات."
...
async def set_allowed_channels_slash(interaction: discord.Interaction,
    ...
    embed = discord.Embed(title="✅ تم تحديث القنوات المسموحة", color=discord.Color.green())
    embed.description = f"القنوات المسموحة: {channels_str}"
...
# ----------------------------------------------------------------------
# Command: اوامر (help) - redesigned visually
# ----------------------------------------------------------------------
@bot.tree.command(name="اوامر", description="عرض قائمة بجميع أوامر البوت")
@app_commands.checks.cooldown(1, 5, key=lambda i: (i.user.id, i.command.qualified_name))
async def help_slash(interaction: discord.Interaction):
    embed = discord.Embed(title="📌 **أوامر البوت – التصنيفات**", color=discord.Color.purple())

    # 👤 أوامر الأعضاء
    members_cmds = (
        "`!تحليل` أو `/تسجيل` — تسجيل فصل / عدة فصول.\n"
        "`!أعمالي` أو `/أعمالي` — استعراض أعمالك.\n"
        "`!شغل @member` أو `/شغل` — عرض شغل عضو.\n"
        "`!اسعار` أو `/اسعار` — الأسعار الحالية.\n"
        "`/تقريري` — تقريرك الأسبوعي.\n"
        "`/ملخص_شهري` — ملخصك للشهر الحالي.\n"
        "`/الأعمال` — قائمة الأعمال والمساهمين.\n"
        "`/تعديل` — تعديل آخر سجل أضفته."
    )
    embed.add_field(name="👤 **أوامر الأعضاء**", value=members_cmds, inline=False)

    # 👑 أوامر الإدارة (تسجيل وإدارة)
    admin_cmds = (
        "`/تسجيل_للغير` — تسجيل فصل لعضو آخر.\n"
        "`/حذف` — حذف سجلات (متقدم).\n"
        "`!حذف_الكل` أو `/حذف_الكل` — مسح كل السجلات.\n"
        "`/حذف_كل_الأعمال` — حذف كل الأعمال من القائمة.\n"
        "`/اضافة_عمل` `/حذف_عمل` `/تعديل_عمل` `/عرض_الاعمال` — إدارة الأعمال.\n"
        "`/مكافأة` `/خصم` `/حذف_مكافأة_خصم` — المكافآت والخصومات.\n"
        "`/اضافة_تخصص` `/حذف_تخصص` `/تفعيل_تخصص` `/تعطيل_تخصص` — إدارة التخصصات.\n"
        "`/تعديل_سعر` — تعديل سعر تخصص.\n"
        "`/تحديث_أسعار` — تطبيق الأسعار الجديدة على السجلات.\n"
        "`/تحديد_موعد_الدفع` `/تقرير_دفع` — الدفع الشهري."
    )
    embed.add_field(name="👑 **أوامر الإدارة**", value=admin_cmds, inline=False)

    # ⚙️ إعدادات وأدوات
    settings_cmds = (
        "`/اعدادات` — العملة، الإشعارات، التنبيهات.\n"
        "`/تحديد_قنوات` — تعيين القنوات المسموحة.\n"
        "`/لوحة_التحكم` — نظرة عامة.\n"
        "`/احصائيات` — إحصائيات شاملة.\n"
        "`/سجل` — سجل العمليات الإدارية.\n"
        "`/تصدير` — تصدير Excel.\n"
        "`/رفع_البيانات` — استيراد بيانات JSON."
    )
    embed.add_field(name="⚙️ **الإعدادات والأدوات**", value=settings_cmds, inline=False)

    channels_str = ", ".join([f"#{ch}" for ch in SETTINGS.get("allowed_channels", [])])
    embed.set_footer(text=f"القنوات المسموحة: {channels_str}")
...
async def export_excel(interaction: discord.Interaction):
    ...
    buffer.seek(0)
    embed = discord.Embed(title="📥 تصدير البيانات", color=discord.Color.blurple(), description="تم إنشاء ملف Excel.")
    await interaction.response.send_message(embed=embed, file=discord.File(buffer, filename="work_report.xlsx"))
...
async def bot_settings(interaction: discord.Interaction, العملة: str = None, قناة_الإشعارات: discord.TextChannel = None, قناة_النسخ: discord.TextChannel = None, حد_التنبيه: float = None):
    ...
    await save_settings(SETTINGS)
    embed = discord.Embed(title="⚙️ تم تحديث الإعدادات", color=discord.Color.green())
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def add_work(interaction: discord.Interaction, الاسم: str, بداية_الفصول_المدفوعة: int = None, نشط: bool = True):
    ...
    embed = discord.Embed(title="✅ تمت إضافة العمل", color=discord.Color.green())
    embed.description = f"**العمل:** `{الاسم}`\n**الحالة:** {desc}\n**نشط:** {'✅' if نشط else '❌'}"
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def delete_work(interaction: discord.Interaction, العمل: str):
    ...
    async def delete_with_records(interaction2: discord.Interaction):
        ...
        embed = discord.Embed(title="✅ تم حذف العمل وكل سجلاته", color=discord.Color.red())
        embed.description = f"العمل `{العمل}`، السجلات المحذوفة: {removed}"
    ...
    async def delete_work_only(interaction2: discord.Interaction):
        ...
        embed = discord.Embed(title="✅ تم حذف العمل من القائمة", color=discord.Color.orange())
        embed.description = f"العمل `{العمل}`، السجلات لم تمس."
        await interaction2.followup.send(embed=embed, ephemeral=True)
    ...
    embed = discord.Embed(title="🗑️ حذف العمل", color=discord.Color.dark_red())
    embed.description = f"اختر طريقة الحذف للعمل `{العمل}`:"
...
async def edit_work(interaction: discord.Interaction, العمل: str, الاسم_الجديد: str = None, بداية_الفصول_المدفوعة: int = None, الكل_مدفوع: bool = False, نشط: bool = None):
    ...
    embed = discord.Embed(title="✅ تم تعديل العمل", color=discord.Color.green())
    embed.description = f"**العمل:** `{العمل}`\n" + "\n".join(changed)
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def add_specialty(interaction: discord.Interaction, الاسم: str, السعر: float, نشط: bool = True):
    ...
    embed = discord.Embed(title="✅ تم إضافة تخصص", color=discord.Color.green())
    embed.description = f"التخصص: `{norm_name}`\nالسعر: {SETTINGS.get('currency', '$')}{السعر:.2f}"
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def delete_specialty(interaction: discord.Interaction, الاسم: str):
    ...
    embed = discord.Embed(title="🔴 تم تعطيل تخصص", color=discord.Color.red())
    embed.description = f"التخصص `{norm_name}` تم تعطيله."
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def activate_specialty(interaction: discord.Interaction, الاسم: str):
    ...
    await save_settings(SETTINGS)
    rebuild_prices()
    embed = discord.Embed(title="🟢 تم تفعيل تخصص", color=discord.Color.green())
    embed.description = f"التخصص `{norm_name}` مفعل الآن."
    await interaction.response.send_message(embed=embed, ephemeral=True)
