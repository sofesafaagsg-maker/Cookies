...
async def log_unauthorized(user_id, command_name):
    ...
    await log_audit("محاولة_غير_مصرح_بها", user_id, None, f"محاولة استخدام الأمر {command_name} بدون صلاحية")
...
async def only_allowed_channel(ctx):
    if ctx.author.bot:
        return False  # ignore bot messages entirely
...
# ---------- Work details view (with optional back button) ----------
...
# ---------- New /الأعمال command (replaces /مشاريع) ----------

async def get_works_info(guild: discord.Guild):
    """Build list of works with their contributors from works collection and records"""
    approved_works = await load_works()
    records = await load_records()

    # Create a mapping of work_name to contributors
    ...
    for w in approved_works:
        ...
        for uid_str, count in contributors.items():
            ...
            name = member.display_name if member else "غير معروف"
...
class MemberSelect(discord.ui.Select):
    def __init__(self, work_name, members_info, guild, works_info_callback=None):
        ...
        self.guild = guild
        self.works_info_callback = works_info_callback  # callback to return to works list
        options = []
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        user_name = user.display_name if user else next((name for uid, name in self.members_info if uid == user_id), "غير معروف")
        records = await load_records()
        ...
        # create a back callback that returns to the member select for this work
        async def back_to_members(interaction2: discord.Interaction):
            ...
            # add a "رجوع للقائمة" button to return to works list
        ...
        view_details = WorkDetailsView(
            self.work_name, chapters_details, user_id, user_name,
...
class WorkSelect(discord.ui.Select):
    def __init__(self, works_info, guild, works_info_callback=None):
        ...
        self.guild = guild
        self.works_info_callback = works_info_callback  # to return to the works list
        options = []
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view = discord.ui.View(timeout=60)
        view.add_item(select)
        # add a "رجوع للقائمة" button to go back to the works list
        if self.works_info_callback:
...
class WorksPaginator(discord.ui.View):
    ...
    async def show_works_list(self, interaction: discord.Interaction):
        """Return to the initial works list page (page 0)"""
...
async def add_work(interaction: discord.Interaction, الاسم: str, بداية_الفصول_المدفوعة: int = None, نشط: bool = True):
    ...
    new_work = {
        "name": الاسم,
        "paid_start": بداية_الفصول_المدفوعة,
        "active": نشط
    }
